import CloseButton from "@/components/button/CloseButton";
import SubmitButton from "@/components/button/SubmitButton";
import FormInput from "@/components/input/FormInput";
import HeaderBar from "@/components/navigation/HeaderBar";
import { addData, showData, updateData } from "@/utils/ApiServices";
import React, { useEffect, useState } from "react";
import { useHistory, useParams } from "react-router";

const DetailPemeriksaan = ({ title, ...props }) => {
  let { parent, id } = useParams();
  let history = useHistory();
  const [data, setData] = useState({
    nama: "",
    nilai_rujukan: "",
    test_id: 0,
    harga: "",
  });
  const [loading, setLoading] = useState(true);
  const [uploading, setUploading] = useState(false);
  const [disabled, setDisabled] = useState(false);

  useEffect(() => {
    let isSubscribed = true;

    if (id !== undefined) {
      showData("examinations", id)
        .then((res) => {
          setData(res);
          setLoading(false);
          console.log(res);
        })
        .catch((err) => console.log(err));
    } else {
      setLoading(false);
    }

    if (isSubscribed) {
      title === "show" && setDisabled(true);
    }

    return () => {
      isSubscribed = false;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleChange = (e) => {
    setData({ ...data, [e.target.name]: e.target.value });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    setUploading(true);
    data.test_id = parent;
    console.log(data);
    if (title === "add") {
      addData("examinations", data).then((res) => {
        console.log(res);
        setUploading(false);
        history.replace(`/daftar-tes/lihat/${parent}`);
      });
    } else if (title === "edit") {
      updateData("examinations", data, id).then((res) => {
        console.log(res);
        setUploading(false);
        history.replace(`/daftar-tes/lihat/${parent}`);
      });
    }
  };

  return (
    <div className="min-h-screen bg-yellow-400 pb-8">
      <HeaderBar>
        {(title === "add" && "Tambah Data Tes") ||
          (title === "edit" && "Ubah Data Tes") ||
          (title === "show" && "Lihat Data Tes")}
      </HeaderBar>
      <div className="mx-4 my-2 px-10 py-5 bg-gray-50 rounded-lg">
        <div className="flex justify-end mb-10">
          <CloseButton backTo={`/daftar-tes/lihat/${parent}`} />
        </div>

        <form onSubmit={handleSubmit}>
          <FormInput
            label="Jenis Pemeriksaan"
            required={true}
            name="nama"
            onChange={handleChange}
            disabled={disabled}
            defaultValue={data.nama}
          />
          <FormInput
            label="Harga"
            required={true}
            type="number"
            name="harga"
            onChange={handleChange}
            disabled={disabled}
            defaultValue={data.harga}
          />
          <FormInput
            label="Nilai Rujukan"
            required={true}
            name="nilai_rujukan"
            onChange={handleChange}
            disabled={disabled}
            defaultValue={data.nilai_rujukan}
          />
          {!(title === "show") && <SubmitButton loading={uploading} />}
        </form>
      </div>
    </div>
  );
};

export default DetailPemeriksaan;
